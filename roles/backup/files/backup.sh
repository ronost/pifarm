...
rsync -av --size-only --exclude={".*",".*/","*.CR2"} /mnt/backup_source_ronny/ /mnt/backup_destination/backup/ronny/
echo "Backup source \"ronny\" end: $(date +%Y-%m-%d" "%H:%M:%S)"
echo "---------------------------------------------------"
echo "Backup source \"bella\" start: $(date +%Y-%m-%d" "%H:%M:%S)"
rsync -av --size-only --exclude={".*",".*/","*.CR2"} /mnt/backup_source_bella/ /mnt/backup_destination/backup/bella/
